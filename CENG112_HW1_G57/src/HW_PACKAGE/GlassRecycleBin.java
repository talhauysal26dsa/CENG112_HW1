...
public class GlassRecycleBin<T> implements IBag<T> {
	...
	public GlassRecycleBin(int capacity) {
	...
	public boolean add(T newItem) {
		...
		if (isFull()) {
			...
			glassBin[numberOfItems] = newItem;
			numberOfItems ++;
	...
	public boolean isFull() {
		...
		return numberOfItems >= glassBin.length;
	}
	...
	public T removeByIndex(int index) {
		T result = null;
		if (!isEmpty()&&index>=0) {
			result = glassBin[index];
			glassBin[index] = glassBin[numberOfItems-1];
			glassBin[numberOfItems-1] = null;
			numberOfItems --;
		}
		return result;
	}
	...
	public T remove() {
		...
		T result = null;
		if (!isEmpty()) {
			result = glassBin[numberOfItems-1];
			glassBin[numberOfItems-1] = null;
			numberOfItems --;
		}
		return result;
